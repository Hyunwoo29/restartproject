...
import { deleteTodoAction, toggleTodoAction } from 'store/Todoreducer'
...
const TodoList = () => {
    ...
    return (<>
    {todos.length === 0 && (<p className='alert alert-info'>등록된 할일 없음</p>)}
    {todos.length !== 0
    && todos.map(todo => (<div key = {todo.id} className = "row mb-1">
        <div className="col-sm-2"><input type="checkbox" checked={todo.complete} onChange={toggleTodo.bind(null, todo.id)}/>
        {todo.complete
